...
class AdaCare(nn.Module):
    def __init__(self, hidden_dim=128, kernel_size=2, kernel_num=64, input_dim=76, output_dim=1, dropout=0.5, r_v=4, r_c=4, activation='sigmoid', device='cuda'):
        ...
        #1224 self-calibrated convolution###############################################
        self.nn_conv_k1 = CausalConv1d(input_dim, input_dim, kernel_size, 1, 1)
        self.nn_conv_k2 = CausalConv1d(input_dim//2, input_dim//2, kernel_size, 1, 1)
        # self.nn_conv_k3 = CausalConv1d(input_dim, input_dim, kernel_size, 1, 1)
        self.nn_conv_k3 = CausalConv1d(input_dim, kernel_num, kernel_size, 1, 1)

        torch.nn.init.xavier_uniform_(self.nn_conv_k1.weight)
        torch.nn.init.xavier_uniform_(self.nn_conv_k2.weight)
        torch.nn.init.xavier_uniform_(self.nn_conv_k3.weight)
        ################################################################################

        #1221 add downsample and upsample#################################
        ...
        # torch.nn.init.xavier_uniform_(self.upsamle.weight)
        ##################################################################
        # self.nn_conv_b_k1 = CausalConv1d(input_dim, input_dim, kernel_size, 1, 1)
        # self.nn_conv_b_k2 = CausalConv1d(input_dim // 2, input_dim // 2, kernel_size, 1, 1)
        # self.nn_conv_b_k3 = CausalConv1d(input_dim, input_dim, kernel_size, 1, 1)
        # # self.nn_conv_k3 = CausalConv1d(input_dim, kernel_num, kernel_size, 1, 1)
        #
        # torch.nn.init.xavier_uniform_(self.nn_conv_k1.weight)
        # torch.nn.init.xavier_uniform_(self.nn_conv_k2.weight)
        ...
        # 1221 add downsample and upsample#################################
        self.downsample_b = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        self.upsamle_b = nn.Upsample(scale_factor=2)
        ...
        # self.rnn = nn.GRUCell(2*input_dim, hidden_dim)
        self.rnn = nn.GRUCell(input_dim+kernel_num, hidden_dim)
    ...
    def forward(self, input, device):
        ...
        #20201224 CVPR2020 self-calibarated convolution
        conv_input = input.permute(0,2,1)
        x1 = self.nn_conv_k1(conv_input)
        x2_in = self.downsample(input)
        x2_in = x2_in.permute(0,2,1)
        x2_out = self.nn_conv_k2(x2_in)
        x2_out = x2_out.permute(0,2,1)
        x2_out = self.upsamle(x2_out)
        x2_out += input
        x2_out = x2_out.permute(0,2,1)
        x2_out = self.sigmoid(x2_out)
        x3_in = x2_out * x1
        x3_out = self.nn_conv_k3(x3_in)
        conv_res = self.relu(x3_out)
        ####################################################
        #20201230 cascade层
        # x1 = self.nn_conv_b_k1(conv_input)
        # x2_in = self.downsample_b(input)
        # x2_in = x2_in.permute(0, 2, 1)
        # x2_out = self.nn_conv_b_k2(x2_in)
        # x2_out = x2_out.permute(0, 2, 1)
        # x2_out = self.upsamle_b(x2_out)
        # x2_out += input
        # x2_out = x2_out.permute(0, 2, 1)
        # x2_out = self.sigmoid(x2_out)
        # x3_in = x2_out * x1
        # x3_out = self.nn_conv_b_k3(x3_in)
        # conv_res_b = self.relu(x3_out)
        #
        # conv_res = torch.cat((conv_res_a, conv_res_b), dim=1)
        ###################################################

        #first just add a simple elastic
        # elastic_input = input
        # elastic = self.downsample(elastic_input)
        # # elastic = self.downsample(elastic)
        # # elastic = self.upsamle(elastic)
        # elastic = self.upsamle(elastic)
        # elastic = elastic.permute(0, 2, 1)
        # elastic = self.nn_conv1(elastic)
        #
        # conv_res1 = conv_res1 + elastic
        # conv_res3 = conv_res1 + elastic
        # conv_res5 = conv_res1 + elastic
        # # conv_res3 += elastic
        # # conv_res5 += elastic
        ########################################
        ...
        for cur_time in range(time_step):
            convse_res = conv_res[:, :, :cur_time + 1]
            inputse_res = input[:, :cur_time + 1, :].permute(0, 2, 1)
            cur_input = torch.cat((convse_res[:, :, -1], inputse_res[:, :, -1]), dim=-1)
